...
| `/teams` | 팀 목록 | 로그인 | 전체 팀 목록, 팀 생성, 공개/비밀번호/초대코드 가입 |
| `/teams/:teamId` | 팀 대시보드 | 팀원 | 팀 요약, task/회고 요약 |
| `/teams/:teamId/members` | 팀원 관리 | 팀원 | 팀원 목록, 팀장 기능 |
| `/teams/:teamId/settings` | 팀 설정 | 팀원 | 팀 정보 조회, 팀장 전용 수정/초대코드 재발급 |
| `/teams/:teamId/tasks` | Task 목록 | 팀원 | task 목록, 필터, 생성 |
| `/teams/:teamId/tasks/:taskId` | Task 상세 | 팀원 | task 수정, 완료 변경, 댓글 |
| `/teams/:teamId/meetings` | 회의 목록 | 팀원 | 회의록 목록, 생성 |
| `/teams/:teamId/meetings/:meetingId` | 회의록 상세 | 팀원 | 회의록 조회/수정/삭제 |
...
| 작성자 | Y | Y | Y | Y |
| 공동 작업자 | Y | Y | Y | N |
| 팀장 | Y | 작성자/공동 작업자인 경우만 | 작성자/공동 작업자인 경우만 | 작성자인 경우만 |

정책:

- 팀장은 팀장이라는 이유만으로 타인의 회고록을 수정/삭제할 수 없다.
- 작성자만 공동 작업자 목록을 변경할 수 있다.
- 공동 작업자는 회고록 본문만 수정할 수 있다.
...
sequenceDiagram
  ...
  API-->>Client: 전체 팀 목록
  User->>Client: 초대코드 입력
  Client->>API: POST /api/teams/join-by-invite
  API-->>Client: MEMBER 멤버십
  Client->>Client: /teams/{teamId} 이동
```
...
| 6 | 댓글 | task 협업 기능 |
| 7 | 회의 목록/상세 | 스펙 문서 자동 생성의 입력 데이터 |
| 8 | 회고록 목록/상세 | 스크럼 일지 기능 |
| 9 | 팀 설정 | 팀장 전용 관리 기능 |
