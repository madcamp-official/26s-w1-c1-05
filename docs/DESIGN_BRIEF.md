...
`design/_ds`에 포함된 high-fidelity 디자인 산출물은 reference prototype으로 유지한다. 실제 API endpoint 매칭은 디자인 HTML을 직접 수정하는 방식이 아니라 React 페이지, API client, view model adapter에서 처리한다.
...
heeseo 브랜치 디자인 산출물 상태:

- `design/_ds/Scrum Helper.dc.html`은 실제 endpoint와 연결되지 않은 인터랙티브 프로토타입이다.
- 내부 `teamList`, `tasks`, `meetings`, `retros`, `specs`는 샘플 state다.
- 실제 서비스 연동은 `frontend/src/api/*Api.ts`와 React page 컴포넌트가 담당한다.
- 디자인 산출물은 최대한 수정하지 않고, 토큰/레이아웃/컴포넌트 표현만 실제 앱에 이식한다.
...
- 디자인 프로토타입의 `POST /teams/join` 같은 문구는 실제 API path가 아니다. 실제 초대코드 가입은 `POST /api/teams/join-by-invite`를 사용한다.
- 디자인 프로토타입의 `SM-142`, `MTG-08`, `SPEC-2`는 표시용 문자열이다. 실제 API mutation에는 숫자 `id`를 사용한다.
- 디자인 프로토타입의 `Backlog / In progress / Done`은 UI column이다. 실제 task 상태는 `completed` boolean뿐이다.
- API 응답 DTO를 직접 디자인 컴포넌트에 밀어 넣지 말고, page 단위 view model로 변환한다.

권장 view model 예시:

| API DTO | 디자인 view model | 변환 기준 |
|---|---|---|
| `TaskResponse` | `TaskCardView` | `id`, `title`, `priority`, `dueDate`, `completed`, `assignees`를 카드 표시값으로 변환 |
| `TeamDashboardResponse` | `DashboardMetricsView` | KPI, progress, growth tree count 계산 |
| `MeetingResponse` | `MeetingListItemView` | 제목, 작성자, 날짜, 요약 preview 생성 |
| `SpecDocumentResponse` | `SpecCardView` | 문서 title, source count, updatedAt, content preview 생성 |
