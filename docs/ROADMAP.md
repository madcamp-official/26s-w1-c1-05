...
- 생성 결과 수정 후 저장: 구현 완료
- 스펙 문서 기반 task 추천 API: 구현 완료
- 추천 task 중 선택한 항목만 실제 task로 생성하는 수락 API: 구현 완료
- task 추천/수락 프론트 UI: 미연결
...
상태: 리더보드/명성 백엔드 API 구현 완료, 시각화 UI 미연결
...
- 리더보드: 유저별 완료 task 수 집계: API 구현 완료
- 명성 시스템: 누적 완료 task 수에 따른 등급 부여: API 구현 완료
...
상태: task dependency와 mock notification API 구현 완료, 로드맵/하위 task UI 미구현
...
- Task 전후 관계 설정: API 구현 완료
- 선행 task 완료 시 후행 task 담당자에게 mock notification event 생성: 구현 완료
- 실제 이메일 알림
- Task 그룹과 하위 task
- 로드맵 Overview 화면
...
| 순위 | 기능 | 이유 |
|---:|---|---|
| 1 | 새 백엔드 API 프론트 client/type 연결 | 구현된 API를 팀원 프론트에서 사용할 수 있게 해야 함 |
| 2 | MySQL 기반 통합 smoke test | 제출 데모 안정성 확인 |
| 3 | 제출 산출물 최종 동기화 | API/DB/테스트/실행 문서와 코드 일치 필요 |
| 4 | KCloud 준비 | 로컬 완성 후 배포 전환 비용 감소 |
| 5 | 업적/성장 나무/로드맵 UI | 제출 안정화 이후 확장 |
...
1. 백엔드 회귀 테스트와 Phase 2~4 백엔드 API는 구현 완료 상태다.
2. 다음 작업은 `docs/CURRENT_WORK_PLAN.md`를 기준으로 진행한다.
3. 새 API의 프론트 client/type 연결을 먼저 수행한다.
4. MySQL 기반 통합 smoke test를 수행한다.
5. 제출 산출물을 최종 동기화한다.
