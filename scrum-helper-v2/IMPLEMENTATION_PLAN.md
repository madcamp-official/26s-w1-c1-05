...
- 전체 팀 목록, 팀 생성, 공개 팀 가입, 비밀번호 팀 가입이 동작한다.
...
필수 설계 문서와 Day 1 인증 기반 스캐폴딩이 준비되었으므로 다음 단계는 팀 기능 구현이다.

1. `backend/`: Team, TeamMember Entity와 Repository 추가
2. `backend/`: 팀 생성, 팀 목록, 팀 가입, 팀 대시보드 API 구현
3. `frontend/`: 팀 목록 화면, 팀 생성 모달, 팀 가입 모달 API 연동
4. `docs/RUNBOOK.md`: 팀 기능 구현 후 API 테스트 예시 추가
